			...
			spawn(1)
				...
				if(AM)
					...
					if (soft || (!blocked && !(areacheck.name == "Space")))
						AM.Move(floorbelow)
						if (!soft && istype(AM, /mob/living/carbon/pony))
							...
							H.apply_damage(min(rand(-damage,damage),0), BRUTE, "l_backleg")
							H.apply_damage(min(rand(-damage,damage),0), BRUTE, "r_backleg")
							H.apply_damage(min(rand(-damage,damage),0), BRUTE, "r_foreleg")
							H.apply_damage(min(rand(-damage,damage),0), BRUTE, "r_foreleg")
