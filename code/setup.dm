...
#define pony_NEEDED_OXYGEN (MOLES_CELLSTANDARD * BREATH_PERCENTAGE * 0.16)
...
#define TEMPERATURE_DAMAGE_COEFFICIENT  1.5 // This is used in handle_temperature_damage() for ponys, and in reagents that affect body temperature. Temperature damage is multiplied by this amount.
#define BODYTEMP_AUTORECOVERY_DIVISOR   12  // This is the divisor which handles how much of the temperature difference between the current body temperature and 310.15K (optimal temperature) ponys auto-regenerate each tick. The higher the number, the slower the recovery. This is applied each tick, so long as the mob is alive.
...
#define BODYTEMP_COLD_DIVISOR           6   // Similar to the BODYTEMP_AUTORECOVERY_DIVISOR, but this is the divisor which is applied at the stage that follows autorecovery. This is the divisor which comes into play when the pony's loc temperature is lower than their body temperature. Make it lower to lose bodytemp faster.
#define BODYTEMP_HEAT_DIVISOR           6   // Similar to the BODYTEMP_AUTORECOVERY_DIVISOR, but this is the divisor which is applied at the stage that follows autorecovery. This is the divisor which comes into play when the pony's loc temperature is higher than their body temperature. Make it lower to gain bodytemp faster.
...
#define BODYTEMP_HEAT_DAMAGE_LIMIT 360.15 // The limit the pony body can take before it starts taking damage from heat.
#define BODYTEMP_COLD_DAMAGE_LIMIT 260.15 // The limit the pony body can take before it starts taking damage from coldness.
...
#define pony_STRIP_DELAY        40   // Takes 40ds = 4s to strip someone.
...
// Used with pony/proc/get_heat_protection() and pony/proc/get_cold_protection().
...
#define BE_MALF       16
#define BE_REV        32
#define BE_CHANGELING_INFILTRATOR      64
#define BE_AI         128
...
var/list/be_special_flags = list(
	...
	"Changeling Infiltrator"        = BE_CHANGELING_INFILTRATOR,
...
// For secHUDs and medHUDs and variants. The number is the location of the image on the list hud_list of ponys.
