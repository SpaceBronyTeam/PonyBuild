...
datum/preferences
	...
	var/b_skin = 200
	var/r_skin_head = 200
	var/g_skin_head = 200				//Head skin color
	var/b_skin_head = 200
	var/r_skin_claws = 200
	var/g_skin_claws = 200				//Claws skin color
	var/b_skin_claws = 200
	var/r_skin_beak = 200
	var/g_skin_beak = 200				//Claws skin color
	var/b_skin_beak = 200
...
/datum/preferences/proc/ShowChoices(mob/user)
	...
	//dat += "Skin Tone: <a href='?_src_=prefs;preference=s_tone;task=input'>[-s_tone + 35]/220<br></a>"//ѕони не нужно
	...
	if(current_species && current_species.flags & HAS_CUTIEMARK)
		dat += "<br>[local("CutieMark")]:<br>"
		if(custom_cutiemark)
	...
	dat += "<br>[local("Backpack Type")]:<br><a href ='?_src_=prefs;preference=bag;task=input'><b>[backbaglist[backbag]]</b></a><br>"
	...
	if(current_species)	for(var/path in current_species.has_external_organ)
		var/list/params = current_species.has_external_organ[path]
		switch(params[2])
			if("skin")
				if(!(params[2] in checklist))
					dat += " <font face='fixedsys' size='3' color='#[num2hex(r_skin, 2)][num2hex(g_skin, 2)][num2hex(b_skin, 2)]'><table style='display:inline;' bgcolor='#[num2hex(r_skin, 2)][num2hex(g_skin, 2)][num2hex(b_skin)]'><tr><td><a href='?_src_=prefs;preference=skin;task=input'>__</a></td></tr></table></font>   skin<br>"
					checklist += params[2]
			if("skin head")
				if(!(params[2] in checklist))
					dat += " <font face='fixedsys' size='3' color='#[num2hex(r_skin_head, 2)][num2hex(g_skin_head, 2)][num2hex(b_skin_head, 2)]'><table style='display:inline;' bgcolor='#[num2hex(r_skin_head, 2)][num2hex(g_skin_head, 2)][num2hex(b_skin_head)]'><tr><td><a href='?_src_=prefs;preference=skin_head;task=input'>__</a></td></tr></table></font>   head<br>"
					checklist += params[2]
			if("skin claw")
				if(!(params[2] in checklist))
					dat += " <font face='fixedsys' size='3' color='#[num2hex(r_skin_claws, 2)][num2hex(g_skin_claws, 2)][num2hex(b_skin_claws, 2)]'><table style='display:inline;' bgcolor='#[num2hex(r_skin_claws, 2)][num2hex(g_skin_claws, 2)][num2hex(b_skin_claws)]'><tr><td><a href='?_src_=prefs;preference=skin_claws;task=input'>__</a></td></tr></table></font>   claws<br>"
					checklist += params[2]
			if("skin beak")
				if(!(params[2] in checklist))
					dat += " <font face='fixedsys' size='3' color='#[num2hex(r_skin_beak, 2)][num2hex(g_skin_beak, 2)][num2hex(b_skin_beak, 2)]'><table style='display:inline;' bgcolor='#[num2hex(r_skin_beak, 2)][num2hex(g_skin_beak, 2)][num2hex(b_skin_beak)]'><tr><td><a href='?_src_=prefs;preference=skin_beak;task=input'>__</a></td></tr></table></font>   beak<br>"
					checklist += params[2]
...
/datum/preferences/proc/SetSpecies(mob/user)
	...
	var/list/check_list = list()//ƒл€ проверки дублировани€
	...
	switch(href_list["task"])
		...
		if("input")
			switch(href_list["preference"])
				...
				if("skin_head")
					var/new_skin = input(user, "Choose your character's head colour: ", "Character Preference", rgb(r_skin, g_skin, b_skin)) as color|null
					if(new_skin)
						r_skin_head = hex2num(copytext(new_skin, 2, 4))
						g_skin_head = hex2num(copytext(new_skin, 4, 6))
						b_skin_head = hex2num(copytext(new_skin, 6, 8))

				if("skin_claws")
					var/new_skin = input(user, "Choose your character's claws colour: ", "Character Preference", rgb(r_skin, g_skin, b_skin)) as color|null
					if(new_skin)
						r_skin_claws = hex2num(copytext(new_skin, 2, 4))
						g_skin_claws = hex2num(copytext(new_skin, 4, 6))
						b_skin_claws = hex2num(copytext(new_skin, 6, 8))

				if("skin_beak")
					var/new_skin = input(user, "Choose your character's beak colour: ", "Character Preference", rgb(r_skin, g_skin, b_skin)) as color|null
					if(new_skin)
						r_skin_beak = hex2num(copytext(new_skin, 2, 4))
						g_skin_beak = hex2num(copytext(new_skin, 4, 6))
						b_skin_beak = hex2num(copytext(new_skin, 6, 8))

				if("backcolor")
					var/new_skin = input(user, "Choose background colour: ", "Character Preference", rgb(r_skin, g_skin, b_skin)) as color|null
					if(new_skin)
						r_backcolor = hex2num(copytext(new_skin, 2, 4))
						g_backcolor = hex2num(copytext(new_skin, 4, 6))
						b_backcolor = hex2num(copytext(new_skin, 6, 8))
				...
				if("limbs")
					...
					switch(limb_name)
						if("Left Leg")
							limb = "l_backleg"
							second_limb = "l_backhoof"
						if("Right Leg")
							limb = "r_backleg"
							second_limb = "r_backhoof"
						if("Left Arm")
							limb = "r_foreleg"
							second_limb = "l_forehoof"
						if("Right Arm")
							limb = "r_foreleg"
							second_limb = "r_forehoof"
						if("Left Foot")
							limb = "l_backhoof"
							third_limb = "l_backleg"
						if("Right Foot")
							limb = "r_backhoof"
							third_limb = "r_backleg"
						if("Left Hand")
							limb = "l_forehoof"
							third_limb = "r_foreleg"
						if("Right Hand")
							limb = "r_forehoof"
							third_limb = "r_foreleg"
...
/datum/preferences/proc/copy_to(mob/living/carbon/pony/character, safety = 0)
	...
	character.r_skin_head = r_skin_head
	character.g_skin_head = g_skin_head
	character.b_skin_head = b_skin_head
	character.r_skin_claws = r_skin_claws
	character.g_skin_claws = g_skin_claws
	character.b_skin_claws = b_skin_claws
	character.r_skin_beak = r_skin_beak
	character.g_skin_beak = g_skin_beak
	character.b_skin_beak = b_skin_beak
