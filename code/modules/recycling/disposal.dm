...
// Can hold items and pony size things, no other draggables
...
// ai as pony but can't flush
/obj/machinery/disposal/attack_ai(mob/user as mob)
	interact(user, 1)

// pony interact with machine
	...
	proc/init(var/obj/machinery/disposal/D)
		...
		for(var/atom/movable/AM in D)
			...
			if(istype(AM, /mob/living/carbon/pony))
				...
				if(FAT in H.mutations)		// is a pony and fat?
	...
	proc/move()
		...
		while(active)
			...
			if(has_fat_guy && prob(2)) // chance of becoming stuck per segment if contains a fat guy
				active = 0
				// find the fat guys
				for(var/mob/living/carbon/pony/H in src)
