...
/obj/item/projectile/change/proc/wabbajack (mob/M as mob in living_mob_list)
	if(istype(M, /mob/living) && M.stat != DEAD)
		...
		var/randomize = pick("monkey","robot","slime","xeno","Earthpony")
		switch(randomize)
			if("monkey")
			...
			if("xeno")
				...
				new_mob.universal_speak = 1
			if("Earthpony")
				new_mob = new /mob/living/carbon/pony(M.loc, pick(all_species))
				if(M.gender == MALE)
					new_mob.gender = MALE
				...
				var/datum/preferences/A = new()	//Randomize appearance for the pony
