...
/obj/machinery/singularity/proc/eat()
	...
	for(var/atom/X in orange(grav_pull,src))
		...
		if(dist > consume_range && istype(X, /atom/movable))
			...
			if(((X) &&(!X:anchored) && (!istype(X,/mob/living/carbon/pony)))|| (src.current_size >= 9))
				step_towards(X,src)
			else if(istype(X,/mob/living/carbon/pony))
				var/mob/living/carbon/pony/H = X
...
/obj/machinery/singularity/narsie/proc/pickcultist() //Narsie rewards his cultists with being devoured first, then picks a ghost to follow. --NEO
	...
	for(var/mob/living/carbon/pony/food in living_mob_list)
		if(food.stat)
			continue
	...
	if(cultists.len)
		acquire(pick(cultists))
		return
		//no living cultists, pick a living pony instead.
	...
	if(cultists.len)
		acquire(pick(cultists))
		return
		//no living ponys, follow a ghost instead.
