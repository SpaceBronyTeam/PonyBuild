...
/mob/living/silicon/robot/Bump(atom/movable/AM as mob|obj, yes)
	spawn( 0 )
		...
		if(ismob(AM))
			var/mob/tmob = AM
			if(istype(tmob, /mob/living/carbon/pony) && (FAT in tmob.mutations))
				if(prob(20))
		...
		if(module.type == /obj/item/weapon/robot_module/janitor)
			...
			if(isturf(tile))
				...
				for(var/A in tile)
					...
					else if(istype(A, /mob/living/carbon/pony))
						var/mob/living/carbon/pony/cleaned_pony = A
						if(cleaned_pony.lying)
							if(cleaned_pony.head)
								cleaned_pony.head.clean_blood()
								cleaned_pony.update_inv_head(0)
							if(cleaned_pony.wear_suit)
								cleaned_pony.wear_suit.clean_blood()
								cleaned_pony.update_inv_wear_suit(0)
							else if(cleaned_pony.w_uniform)
								cleaned_pony.w_uniform.clean_blood()
								cleaned_pony.update_inv_w_uniform(0)
							if(cleaned_pony.shoes)
								cleaned_pony.shoes.clean_blood()
								cleaned_pony.update_inv_shoes(0)
							cleaned_pony.clean_blood(1)
							cleaned_pony << "\red [src] cleans your face!"
		return
