...
/mob/living/carbon/attack_hand(mob/M as mob)
	...
	if (hasorgans(M))
		var/datum/organ/external/temp = M:organs_by_name["r_forehoof"]
		if (M.hand)
			temp = M:organs_by_name["l_forehoof"]
...
/mob/living/carbon/proc/help_shake_act(mob/living/carbon/M)
	if (src.health >= config.health_threshold_crit)
		if(src == M && istype(src, /mob/living/carbon/pony))
			var/mob/living/carbon/pony/H = src
			src.visible_message( \
			...
			if (src.gender == MALE)
				...
				t_him = "her"
			if (istype(src,/mob/living/carbon/pony) && src:w_uniform)
				var/mob/living/carbon/pony/H = src
...
/mob/living/carbon/throw_item(atom/target)
	...
	var/atom/movable/item = hand.item_in_hand
...
/mob/living/carbon/Bump(atom/movable/AM as mob|obj, yes)
	...
	spawn( 0 )
		...
		if(ismob(AM))
			...
			if(istype(tmob, /mob/living/carbon/pony))
			...
			if(istype(tmob, /mob/living/carbon/pony) && (FAT in tmob.mutations))
				if(prob(40) && !(FAT in src.mutations))
					...
					now_pushing = 0
					return
			if(tmob.type_in_hands(/obj/item/weapon/shield/riot))
				if(prob(99))
					now_pushing = 0
					return
			if(!(tmob.status_flags & CANPUSH))
