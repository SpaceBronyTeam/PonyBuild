...
/mob/living/carbon/pony/verb/quick_equip()
...
/mob/living/carbon/pony/proc/equip_in_one_of_slots(obj/item/W, list/slots, del_on_fail = 1)
...
/mob/living/carbon/pony/proc/has_organ(name)
	var/datum/organ/external/O = organs_by_name[name]

	return (O && !(O.status & ORGAN_DESTROYED) )

/mob/living/carbon/pony/proc/has_organ_for_slot(slot)
	switch(slot)
		if(slot_back)
		...
		if(slot_handcuffed)
			return has_organ("l_forehoof") && has_organ("r_forehoof")
		if(slot_legcuffed)
			return has_organ("l_backleg") && has_organ("r_backleg")
		if(slot_l_hand)
			return has_organ("l_forehoof")
		if(slot_r_hand)
			return has_organ("r_forehoof")
		if(slot_belt)
			return has_organ("chest")
		if(slot_wear_id)
		...
		if(slot_head)
			return has_organ("head")
		if(slot_shoes)
			return has_organ("r_backhoof") && has_organ("l_backhoof")
		if(slot_wear_suit)
			return has_organ("chest")
		if(slot_w_uniform)
			return has_organ("chest")
...
/mob/living/carbon/pony/u_equip(obj/item/W as obj)
	if(!W)	return 0
...
/obj/effect/equip_e/pony
	name = "Earthpony"
	var/mob/living/carbon/pony/target = null
...
/obj/effect/equip_e/pony/process()
	if (item)
		...
		if(src)	switch(place)
			...
			if("splints")
				var/count = 0
				for(var/organ in list("l_backleg","r_backleg","r_foreleg","r_foreleg"))
...
This proc works for ponys only. Aliens stripping ponys and the like will all use this proc. Stripping monkeys or somesuch will use their version of this proc.
...
/obj/effect/equip_e/pony/done()	//TODO: And rewrite this :< ~Carn
	...
	switch(place)	//here we go again...
		...
		if("handcuff")
			...
			else if (source != target && ispony(source))
