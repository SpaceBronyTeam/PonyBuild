...
/datum/unarmed_attack/proc/is_usable(var/mob/living/carbon/pony/user)
	if(user.restrained())
		return 0

	// Check if they have a functioning hand.
	var/datum/organ/external/E = user.organs_by_name["l_forehoof"]
	if(E && !(E.status & ORGAN_DESTROYED))
		return 1

	E = user.organs_by_name["r_forehoof"]
	if(E && !(E.status & ORGAN_DESTROYED))
		return 1
...
/datum/unarmed_attack/proc/apply_effects(var/mob/living/carbon/pony/user,var/mob/living/carbon/pony/target,var/armour,var/attack_damage,var/zone)
	...
	if(attack_damage >= 5 && armour < 2 && !(target == user) && stun_chance <= attack_damage * 5) // 25% standard chance
		switch(zone) // strong punches can have effects depending on where they hit
			...
			if("r_foreleg", "l_forehoof", "r_foreleg", "r_forehoof")
				if(target.hand)
					// Disarm left hand
			...
			if("l_backleg", "l_backhoof", "r_backleg", "r_backhoof")
				if(!target.lying)
...
/datum/unarmed_attack/proc/show_attack(var/mob/living/carbon/pony/user, var/mob/living/carbon/pony/target, var/zone, var/attack_damage)
...
/datum/unarmed_attack/bite/is_usable(var/mob/living/carbon/pony/user, var/mob/living/carbon/pony/target, var/zone)
...
/datum/unarmed_attack/punch/show_attack(var/mob/living/carbon/pony/user, var/mob/living/carbon/pony/target, var/zone, var/attack_damage)
	var/datum/organ/external/affecting = target.get_organ(zone)
...
/datum/unarmed_attack/kick/is_usable(var/mob/living/carbon/pony/user, var/mob/living/carbon/pony/target, var/zone)
	if (user.legcuffed)
		return 0

	if(!(zone in list("l_backleg", "r_backleg", "l_backhoof", "r_backhoof", "groin")))
		return 0

	var/datum/organ/external/E = user.organs_by_name["l_backhoof"]
	if(E && !(E.status & ORGAN_DESTROYED))
		return 1

	E = user.organs_by_name["r_backhoof"]
	if(E && !(E.status & ORGAN_DESTROYED))
		return 1

	return 0

/datum/unarmed_attack/kick/get_unarmed_damage(var/mob/living/carbon/pony/user)
	var/obj/item/clothing/shoes = user.shoes
	if(!istype(shoes))
		return damage
	return damage + (shoes ? shoes.force : 0)

/datum/unarmed_attack/kick/show_attack(var/mob/living/carbon/pony/user, var/mob/living/carbon/pony/target, var/zone, var/attack_damage)
	var/datum/organ/external/affecting = target.get_organ(zone)
	var/organ = affecting.display_name
...
/datum/unarmed_attack/stomp/is_usable(var/mob/living/carbon/pony/user, var/mob/living/carbon/pony/target, var/zone)
	...
	if (!user.lying && (target.lying || (zone in list("l_backhoof", "r_backhoof"))))
...
/datum/unarmed_attack/stomp/get_unarmed_damage(var/mob/living/carbon/pony/user)
	var/obj/item/clothing/shoes = user.shoes
	return damage + (shoes ? shoes.force : 0)

/datum/unarmed_attack/stomp/show_attack(var/mob/living/carbon/pony/user, var/mob/living/carbon/pony/target, var/zone, var/attack_damage)
	var/datum/organ/external/affecting = target.get_organ(zone)
	var/organ = affecting.display_name
	var/obj/item/clothing/shoes = user.shoes
