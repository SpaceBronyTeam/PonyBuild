/mob/living/carbon/pony/attack_hand(mob/living/carbon/M as mob)

	var/mob/living/carbon/pony/H = M
	if(istype(H))
		var/datum/organ/external/temp = H.organs_by_name["r_forehoof"]
		if(H.hand)
			temp = H.organs_by_name["l_forehoof"]
	...
	if(istype(H))
		...
		if(istype(H.gloves, /obj/item/clothing/gloves/boxing/hologlove))
			...
			if(!damage)
				...
				return 0
			var/datum/organ/external/affecting = get_organ(ran_zone(H.zone_sel.selecting.name))
	...
	switch(M.a_intent)
		...
		if("hurt")
			...
			var/block = 0
			var/accurate = 0
			var/hit_zone = H.zone_sel.selecting.name
		...
		if("disarm")
			...
			if(w_uniform)
				w_uniform.add_fingerprint(M)
			var/datum/organ/external/affecting = get_organ(ran_zone(M.zone_sel.selecting.name))

			for(var/datum/hand/selhand in list_hands)
				if(istype(selhand.item_in_hand, /obj/item/weapon/gun))
					var/obj/item/weapon/gun/W = null
					var/chance = 0

					W = selhand.item_in_hand
					chance += hand ? 40 : 20
			...
			if(randn <= 60)
				...
				for(var/datum/hand/selhand in list_hands)
					if(istype(selhand.item_in_hand, /obj/item/weapon/grab))
						var/obj/item/weapon/grab/hgrab = selhand.item_in_hand
						if(hgrab.affecting)
							visible_message("\red <b>[M] has broken [src]'s grip on [hgrab.affecting]!</B>")
							talked = 1
						spawn(1)
							del(hgrab)
...
/mob/living/carbon/pony/proc/afterattack(atom/target as mob|obj|turf|area, mob/living/user as mob|obj, inrange, params)
	return

/mob/living/carbon/pony/attack_generic(var/mob/user, var/damage, var/attack_message)
	...
	var/dam_zone = pick("head", "chest", "r_foreleg", "r_foreleg", "l_backleg", "r_backleg", "groin")
