/mob/living/carbon/pony/proc/monkeyize()
	if (monkeyizing)
		return
...
/mob/living/carbon/pony/AIize(move=1) // 'move' argument needs defining here too because BYOND is dumb
	if (monkeyizing)
		return
	for(var/t in organs)
		del(t)
...
//pony -> alien
/mob/living/carbon/pony/proc/Alienize()
	if (monkeyizing)
		return
	for(var/obj/item/W in src)
		drop_from_inventory(W)
	...
	var/mob/living/carbon/pony/new_xeno = create_new_xenomorph(alien_caste,loc)
...
/mob/living/carbon/pony/proc/slimeize(adult as num, reproduce as num)
	if (monkeyizing)
		return
	for(var/obj/item/W in src)
		drop_from_inventory(W)
...
/mob/living/carbon/pony/proc/corgize()
	if (monkeyizing)
		return
	for(var/obj/item/W in src)
		drop_from_inventory(W)
...
/mob/living/carbon/pony/Animalize()
