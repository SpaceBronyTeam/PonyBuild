...
"By entering the game, you are agreeing to our <a href='?_src_=welcome;motd=rules_[client.language]'>rules</a>.")
	...
	Topic(href, href_list[])
		...
		if(href_list["SelectedJob"])
			...
			if(client.prefs.species != "Earthpony")
				if(!is_alien_whitelisted(src, client.prefs.species) && config.usealienwhitelist)
					src << alert("You are currently not whitelisted to play [client.prefs.species].")
					return 0
	...
	proc/AttemptLateSpawn(rank,var/spawning_at)
		...
		var/mob/living/character = create_character()	//creates the pony and transfers vars and mind
		character = job_master.EquipRank(character, rank, 1)					//equips the pony
	...
	proc/AnnounceArrival(var/mob/living/carbon/pony/character, var/rank, var/join_message)
	...
	proc/create_character()
		...
		var/mob/living/carbon/pony/new_character
