	...
	if(isturf(mob.loc))
		...
		if(mob.pulledby || mob.buckled) // Wheelchair driving!
			...
			if(istype(mob.pulledby, /obj/structure/bed/chair/wheelchair))
				...
				if(ispony(mob.buckled))
					var/mob/living/carbon/pony/driver = mob.buckled
					var/datum/organ/external/l_forehoof = driver.get_organ("l_forehoof")
					var/datum/organ/external/r_forehoof = driver.get_organ("r_forehoof")
					if((!l_forehoof || (l_forehoof.status & ORGAN_DESTROYED)) && (!r_forehoof || (r_forehoof.status & ORGAN_DESTROYED)))
	...
	for(var/turf/turf in oview(1,src))
		...
		if(istype(turf,/turf/simulated/floor)) // Floors don't count if they don't have gravity
			...
			if(istype(A) && A.has_gravity == 0)
				...
				if(ispony(src))  // Only ponys can wear magboots, so we give them a chance to.
					var/mob/living/carbon/pony/H = src
