// fun if you want to typecast ponys/monkeys/etc without writing long path-filled lines.
/proc/ispony(A)
	if(istype(A, /mob/living/carbon/pony))
		return 1
	return 0
...
/proc/isxenomorph(A)
	if(istype(A, /mob/living/carbon/pony))
		var/mob/living/carbon/pony/H = A
		return istype(H.species, /datum/species/changeling)
	return 0
...
/mob/living/carbon/pony/isSynthetic()
...
/proc/ran_zone(zone, probability)
	...
	while (ran_zone == zone)
		ran_zone = pick (
			...
			organ_rel_size["r_foreleg"]; "r_foreleg",
			organ_rel_size["r_foreleg"]; "r_foreleg",
			organ_rel_size["l_backleg"]; "l_backleg",
			organ_rel_size["r_backleg"]; "r_backleg",
			organ_rel_size["l_forehoof"]; "l_forehoof",
			organ_rel_size["r_forehoof"]; "r_forehoof",
			organ_rel_size["l_backhoof"]; "l_backhoof",
			organ_rel_size["r_backhoof"]; "r_backhoof",
		)
...
/mob/verb/a_intent_change(input as text)
	...
	if(ispony(src) || isbrain(src) || isslime(src))
		switch(input)
