...
/obj/item/weapon/reagent_containers/food/snacks/grown/attack_self(mob/user as mob)
	...
	if(seed.kitchen_tag)
		switch(seed.kitchen_tag)
			if("shand")
				var/obj/item/stack/medical/bruise_pack/pegasus/poultice = new /obj/item/stack/medical/bruise_pack/pegasus(user.loc)
				poultice.heal_brute = potency
				user << "<span class='notice'>You mash the leaves into a poultice.</span>"
				del(src)
				return
			if("mtear")
				var/obj/item/stack/medical/ointment/pegasus/poultice = new /obj/item/stack/medical/ointment/pegasus(user.loc)
...
/obj/item/weapon/reagent_containers/food/snacks/grown/pickup(mob/user)
	...
	if(seed.get_trait(TRAIT_STINGS))
		...
		seed.do_sting(H,src,pick("r_forehoof","l_forehoof"))
