...
/datum/seed/proc/do_thorns(var/mob/living/carbon/pony/target, var/obj/item/fruit, var/target_limb)
	...
	if(!target_limb) target_limb = pick("l_backhoof","r_backhoof","l_backleg","r_backleg","l_forehoof","r_forehoof","r_foreleg", "r_foreleg","head","chest","groin")
...
/datum/seed/proc/do_sting(var/mob/living/carbon/pony/target, var/obj/item/fruit)
	...
	if(chems && chems.len)
		...
		for(var/obj/item/clothing/clothes in target)
			if(target.item_in_hands(clothes))
				continue
