/datum/admins/Topic(href, href_list)
	...
	if(href_list["makeAntag"])
		...
		switch(href_list["simplemake"])
			...
			if("Earthpony")			M.change_mob_type( /mob/living/carbon/pony , null, null, delmob, href_list["species"])
			if("Pegasus")			M.change_mob_type( /mob/living/carbon/pony , null, null, delmob, href_list["species"])
			if("Unicorn")			M.change_mob_type( /mob/living/carbon/pony , null, null, delmob, href_list["species"])
			if("Alicorn")			M.change_mob_type( /mob/living/carbon/pony , null, null, delmob, href_list["species"])
			if("Griffon")			M.change_mob_type( /mob/living/carbon/pony , null, null, delmob, href_list["species"])
			if("Changeling Queen")			M.change_mob_type( /mob/living/carbon/pony , null, null, delmob, href_list["species"])
			if("Changeling Princess")		M.change_mob_type( /mob/living/carbon/pony , null, null, delmob, href_list["species"])
			if("Changeling Drone")			M.change_mob_type( /mob/living/carbon/pony , null, null, delmob, href_list["species"])
			if("Changeling Sentinel")		M.change_mob_type( /mob/living/carbon/pony , null, null, delmob, href_list["species"])
			if("Changeling Hatchling")		M.change_mob_type( /mob/living/carbon/pony , null, null, delmob, href_list["species"])
			if("Changeling Infiltrator")	M.change_mob_type( /mob/living/carbon/pony , null, null, delmob, href_list["species"])
...
//Regular jobs
	...
	//Non-pony (Green)
		counter = 0
		jobs += "<table cellpadding='1' cellspacing='0' width='100%'>"
		jobs += "<tr bgcolor='ccffcc'><th colspan='[length(nonpony_positions)+1]'><a href='?src=\ref[src];jobban3=nonponydept;jobban4=\ref[M]'>Non-pony Positions</a></th></tr><tr align='center'>"
		for(var/jobPos in nonpony_positions)
			if(!jobPos)	continue
			var/datum/job/job = job_master.GetJob(jobPos)
			if(!job) continue
...
*/
	...
	//JOBBAN'S INNARDS
		...
		var/mob/living/carbon/pony/H = locate(href_list["monkeyone"])
		if(!istype(H))
			usr << "This can only be used on instances of type /mob/living/carbon/pony"
			return
		...
		var/mob/living/carbon/pony/H = locate(href_list["corgione"])
		if(!istype(H))
			usr << "This can only be used on instances of type /mob/living/carbon/pony"
			return
		...
		if(istype(M, /mob/living/carbon/pony))
			var/mob/living/carbon/pony/prisoner = M
		...
		if(istype(M, /mob/living/carbon/pony))
			var/mob/living/carbon/pony/observer = M
		...
		var/mob/living/carbon/pony/H = locate(href_list["makeai"])
		if(!istype(H))
			usr << "This can only be used on instances of type /mob/living/carbon/pony"
			return
		...
		var/mob/living/carbon/pony/H = locate(href_list["makealien"])
		if(!istype(H))
			usr << "This can only be used on instances of type /mob/living/carbon/pony"
			return
		...
		var/mob/living/carbon/pony/H = locate(href_list["makeslime"])
		if(!istype(H))
			usr << "This can only be used on instances of type /mob/living/carbon/pony"
			return
		...
		var/mob/living/carbon/pony/H = locate(href_list["makerobot"])
		if(!istype(H))
			usr << "This can only be used on instances of type /mob/living/carbon/pony"
			return
		...
		var/mob/living/carbon/pony/H = locate(href_list["togmutate"])
		if(!istype(H))
			usr << "This can only be used on instances of type /mob/living/carbon/pony"
			return
...
// Now isn't that much better? IT IS NOW A PROC, i.e. kinda like a big panel like unstable
		...
		var/mob/living/carbon/pony/H = locate(href_list["adminspawncookie"])
		if(!ispony(H))
			usr << "This can only be used on instances of type /mob/living/carbon/pony"
			return
		...
		if(selhand)
			H.put_in_active_hand(new /obj/item/weapon/reagent_containers/food/snacks/cookie(H), selhand)
		else
		...
		var/mob/living/carbon/pony/H = locate(href_list["CentcommReply"])
		if(!istype(H))
			usr << "This can only be used on instances of type /mob/living/carbon/pony"
			return
		...
		var/mob/living/carbon/pony/H = locate(href_list["SyndicateReply"])
		if(!istype(H))
			usr << "This can only be used on instances of type /mob/living/carbon/pony"
			return
		if(!istype(H.l_ear, /obj/item/device/radio/headset) && !istype(H.r_ear, /obj/item/device/radio/headset))
		...
		//TODO
		/*
		var/mob/living/carbon/pony/H = locate(href_list["SolGovFaxReply"])
		var/obj/machinery/photocopier/faxmachine/fax = locate(href_list["originfax"])
		...
		*/
		...
		if ( where == "inhand" )	//Can only give when pony or monkey
			if ( !( ispony(usr) || ismonkey(usr) ) )
				usr << "Can only spawn in hand when you're a pony or a monkey."
				where = "onfloor"
...
/*					for(var/obj/machinery/vehicle/pod/O in world)
			...
			if("monkey")
				...
				for(var/mob/living/carbon/pony/H in mob_list)
					spawn(0)
						H.monkeyize()
				ok = 1
			if("corgi")
...
//don't warp them if they aren't ready or are already there
			...
			if("traitor_all")
				...
				for(var/mob/living/carbon/pony/H in player_list)
...
/*				if("shockwave")
			...
			if("dorf")
				...
				for(var/mob/living/carbon/pony/B in mob_list)
