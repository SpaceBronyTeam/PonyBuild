...
/datum/admins/proc/makeTraitors()
	...
	var/list/mob/living/carbon/pony/candidates = list()
	var/mob/living/carbon/pony/H = null

	for(var/mob/living/carbon/pony/applicant in player_list)
...
/datum/admins/proc/makeWizard()
	...
	if(theghost)
		var/mob/living/carbon/pony/new_character=makeBody(theghost)
	...
	if(candidates.len)
		...
		for (var/obj/effect/landmark/L in /area/syndicate_mothership/elite_squad)
			...
			if (L.name == "Syndicate-Commando")
				...
				var/mob/living/carbon/pony/new_syndicate_commando = create_syndicate_death_commando(L, syndicate_leader_selected)
...
/datum/admins/proc/makeBody(var/mob/dead/observer/G_found) // Uses stripped down and bastardized code from respawn character
	...
	var/mob/living/carbon/pony/new_character = new(pick(latejoin))//The mob being spawned.
...
/datum/admins/proc/create_syndicate_death_commando(obj/spawn_location, syndicate_leader_selected = 0)
	var/mob/living/carbon/pony/new_syndicate_commando = new(spawn_location.loc)
	...
	if(candidates.len)
		...
		for (var/obj/effect/landmark/L in world)
			if(L.name == "voxstart")
				...
				var/mob/living/carbon/pony/new_vox = create_vox_raider(L, leader_chosen)
				...
				new_vox << "\blue You are a Vox Primalis, fresh out of the Shoal. Your ship has arrived at a pony-meat system hosting the NSV Exodus... or was it the Luna? NSS? Utopia? Nobody is really sure, who cares about stupid meat-names anyway? Everyone is raring to start pillaging! Your current goal is: \red<B> [input]</B>"
...
/datum/admins/proc/create_vox_raider(obj/spawn_location, leader_chosen = 0)
	...
	var/mob/living/carbon/pony/new_vox = new(spawn_location.loc, "Vox")
