...
*/
		...
		slimetoxin
			...
			on_mob_life(var/mob/living/M as mob)
				if(!M) M = holder.my_atom
				if(ispony(M))
					var/mob/living/carbon/pony/pony = M
					if(pony.species.name != "Slime")
						M << "<span class='danger'>Your flesh rapidly mutates!</span>"
						pony.set_species("Slime")
				..()
				return
		...
		serotrotium
			name = "Serotrotium"
			id = "serotrotium"
			description = "A chemical compound that promotes concentrated production of the serotonin neurotransmitter in ponys."
			reagent_state = LIQUID
		...
		toxin/acid
			...
			reaction_mob(var/mob/living/M, var/method=TOUCH, var/volume)//magic numbers everywhere
				...
				if(method == TOUCH)
					...
					if(!M.unacidable)
						if(istype(M, /mob/living/carbon/pony) && volume >= 10)
							var/mob/living/carbon/pony/H = M
...
// 	condiments, additives, and such go.
		...
		condensedcapsaicin
			...
			reaction_mob(var/mob/living/M, var/method=TOUCH, var/volume)
				...
				if(method == TOUCH)
					if(istype(M, /mob/living/carbon/pony))
						var/mob/living/carbon/pony/victim = M
						var/mouth_covered = 0
						var/eyes_covered = 0
		...
		sprinkles
			...
			on_mob_life(var/mob/living/M as mob)
				M.nutrition += nutriment_factor
				/*if(istype(M, /mob/living/carbon/pony) && M.job in list("Security Officer", "Head of Security", "Detective", "Warden"))
					if(!M) M = holder.my_atom
					M.heal_organ_damage(1,1)
					M.nutrition += nutriment_factor
					..()
	...
	//removed because of meta bullshit. this is why we can't have nice things.
		syndicream
			...
			on_mob_life(var/mob/living/M as mob)
				M.nutrition += nutriment_factor
				if(istype(M, /mob/living/carbon/pony) && M.mind)
