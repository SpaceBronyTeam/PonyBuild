...
/obj/effect/gibspawner
	...
	var/fleshcolor //Used for gibbed ponys.
	var/bloodcolor //Used for gibbed ponys.
		...
		for(var/i = 1, i<= gibtypes.len, i++)
			if(gibamounts[i])
				for(var/j = 1, j<= gibamounts[i], j++)
					...
					// Apply pony species colouration to masks.
					if(fleshcolor)
					...
					else if(istype(src, /obj/effect/gibspawner/pony)) // Probably a monkey
						gib.blood_DNA["Non-pony DNA"] = "A+"
