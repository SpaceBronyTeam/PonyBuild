...
/obj/structure/attack_hand(mob/user)
	if(breakable)
		...
		else if(istype(user,/mob/living/carbon/pony))
	...
	for(var/mob/living/M in get_turf(src))
		...
		if(prob(25))
			...
			switch(pick(list("ankle","wrist","head","knee","elbow")))
				if("ankle")
					affecting = H.get_organ(pick("l_backhoof", "r_backhoof"))
				if("knee")
					affecting = H.get_organ(pick("l_backleg", "r_backleg"))
				if("wrist")
					affecting = H.get_organ(pick("l_forehoof", "r_forehoof"))
				if("elbow")
					affecting = H.get_organ(pick("r_foreleg", "r_foreleg"))
				if("head")
