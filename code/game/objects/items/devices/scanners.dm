...
/obj/item/device/healthanalyzer/attack(mob/living/M as mob, mob/living/user as mob)
	...
	if (!istype(M, /mob/living/carbon) || (ispony(M) && (M:species.flags & IS_SYNTHETIC)))
	...
	if(istype(M, /mob/living/carbon/pony) && mode == 1)
	...
	if(ispony(M))
		...
		for(var/name in H.organs_by_name)
			...
			if(e.status & ORGAN_BROKEN)
				if(((e.name == "r_foreleg") || (e.name == "r_foreleg") || (e.name == "l_backleg") || (e.name == "r_backleg")) && (!(e.status & ORGAN_SPLINTED)))
...
/obj/item/device/mass_spectrometer/attack_self(mob/user as mob)
	...
	if (crit_fail)
		...
		return
	if (!(istype(user, /mob/living/carbon/pony) || ticker) && ticker.mode.name != "monkey")
