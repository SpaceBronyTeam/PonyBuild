...
/obj/item/weapon/card/id/syndicate/New(mob/user as mob)
	...
	if(!isnull(user)) // Runtime prevention on laggy starts or where users log out because of lag at round start.
		registered_name = ispony(user) ? user.real_name : user.name
	else
...
/obj/item/weapon/card/id/syndicate/attack_self(mob/user as mob)
	if(!src.registered_name)
		...
		var t = reject_bad_name(input(user, "What name would you like to put on this card?", "Agent card name", ispony(user) ? user.real_name : user.name))
		...
		switch(alert("Would you like to display the ID, or retitle it?","Choose.","Rename","Show"))
			if("Rename")
				var t = sanitize(copytext(input(user, "What name would you like to put on this card?", "Agent card name", ispony(user) ? user.real_name : user.name),1,26))
