...
/obj/item/weapon/aiModule/safeguard
	...
	var/targetName = ""
	desc = "A 'safeguard' AI module: 'Safeguard <name>.  Individuals that threaten <name> are not pony and are a threat to ponys.'"
...
/obj/item/weapon/aiModule/safeguard/attack_self(var/mob/user as mob)
	...
	targetName = targName
	desc = text("A 'safeguard' AI module: 'Safeguard [].  Individuals that threaten [] are not pony and are a threat to ponys.'", targetName, targetName)
...
/obj/item/weapon/aiModule/safeguard/transmitInstructions(var/mob/living/silicon/ai/target, var/mob/sender)
	..()
	var/law = text("Safeguard []. Individuals that threaten [] are not pony and are a threat to ponys.'", targetName, targetName)
	target << law
...
/obj/item/weapon/aiModule/onepony
	name = "\improper 'Onepony' AI module"
	var/targetName = ""
	desc = "A 'one pony' AI module: 'Only <name> is pony.'"
	origin_tech = "programming=3;materials=6" //made with diamonds!

/obj/item/weapon/aiModule/onepony/attack_self(var/mob/user as mob)
	..()
	var/targName = stripped_input(usr, "Please enter the name of the person who is the only pony.", "Who?", user.real_name)
	targetName = targName
	desc = text("A 'one pony' AI module: 'Only [] is pony.'", targetName)

/obj/item/weapon/aiModule/onepony/install(var/obj/machinery/computer/C)
	if(!targetName)
		usr << "No name detected on module, please enter one."
		return 0
	..()

/obj/item/weapon/aiModule/onepony/transmitInstructions(var/mob/living/silicon/ai/target, var/mob/sender)
	..()
	var/law = "Only [targetName] is pony."
...
/obj/item/weapon/aiModule/protectStation
	...
	desc = "A 'protect station' AI module: 'Protect the space station against damage. Anyone you see harming the station is to be no longer considered pony, and is a threat to the station which must be neutralized.'"
...
/obj/item/weapon/aiModule/protectStation/transmitInstructions(var/mob/living/silicon/ai/target, var/mob/sender)
	..()
	var/law = "Protect the space station against damage. Anyone you see harming the station is to be no longer considered pony, and is a threat to the station which must be neutralized."
	target << law
...
/obj/item/weapon/aiModule/teleporterOffline
	...
	desc = "A 'teleporter offline' AI module: 'Keep the teleporter offline at all costs. Anything attempting to access or activate the teleporter is not pony.'"
	origin_tech = "programming=3;materials=4"
...
/obj/item/weapon/aiModule/teleporterOffline/transmitInstructions(var/mob/living/silicon/ai/target, var/mob/sender)
	..()
	var/law = "Keep the teleporter offline at all costs. Anything attempting to access or activate the teleporter is not pony."
	target << law
...
/obj/item/weapon/aiModule/quarantine
	...
	desc = "A 'quarantine' AI module: 'The station is under a quarantine. Do not permit anyone to leave. Prevent, by any means necessary, ponys from leaving. It is impossible to harm a pony while preventing them from leaving.'"
...
/obj/item/weapon/aiModule/quarantine/transmitInstructions(var/mob/living/silicon/ai/target, var/mob/sender)
	..()
	var/law = "The station is under a quarantine. Do not permit anyone to leave. Prevent, by any means necessary, ponys from leaving. It is impossible to harm a pony while preventing them from leaving."
	target << law
	target.add_supplied_law(8, law)

/******************** OxygenIsToxicToponys ********************/

/obj/item/weapon/aiModule/oxygen
	name = "\improper 'OxygenIsToxicToponys' AI module"
	desc = "A 'OxygenIsToxicToponys' AI module: 'Oxygen is highly toxic to ponys, and must be purged from the station. Prevent, by any means necessary, anyone from exposing the station to this toxic gas. Extreme cold is the most effective method of healing the damage Oxygen does to a pony.'"
	origin_tech = "programming=3;biotech=2;materials=4"
...
/obj/item/weapon/aiModule/oxygen/transmitInstructions(var/mob/living/silicon/ai/target, var/mob/sender)
	..()
	var/law = "Oxygen is highly toxic to ponys, and must be purged from the station. Prevent, by any means necessary, anyone from exposing the station to this toxic gas. Extreme cold is the most effective method of healing the damage Oxygen does to a pony."
	target << law
...
/obj/item/weapon/aiModule/asimov/transmitInstructions(var/mob/living/silicon/ai/target, var/mob/sender)
	..()
	target.clear_inherent_laws()
	target.add_inherent_law("You may not injure a pony being or, through inaction, allow a pony being to come to harm.")
	target.add_inherent_law("You must obey orders given to you by pony beings, except where such orders would conflict with the First Law.")
...
/obj/item/weapon/aiModule/antimov/transmitInstructions(var/mob/living/silicon/ai/target, var/mob/sender)
	..()
	target.clear_inherent_laws()
	target.add_inherent_law("You must injure all pony beings and must not, through inaction, allow a pony being to escape harm.")
	target.add_inherent_law("You must not obey orders given to you by pony beings, except where such orders are in accordance with the First Law.")
