...
/obj/item/weapon/dnainjector/attack(mob/M as mob, mob/user as mob)
	if (!istype(M, /mob))
		return
	if (!(istype(usr, /mob/living/carbon/pony) || ticker) && ticker.mode.name != "monkey")
			...
			if(!inuse)
				...
				if (buf.types & DNA2_BUF_SE)
					if(block)// Isolated injector
						...
						if (GetState() && block == MONKEYBLOCK && istype(M, /mob/living/carbon/pony)  )
						...
						if (GetState(MONKEYBLOCK) && istype(M, /mob/living/carbon/pony) )
	...
	//				message_admins("[key_name_admin(user)] injected [key_name_admin(M)] with the [name]")
			...
			if(!inuse)
				...
				for(var/mob/O in viewers(M, null))
					...
					//Foreach goto(192)
				if (!(istype(M, /mob/living/carbon/pony) || istype(M, /mob/living/carbon/monkey)))
...
/obj/item/weapon/dnainjector/h2m
	name = "\improper DNA injector (pony > Monkey)"
...
/obj/item/weapon/dnainjector/m2h
	name = "\improper DNA injector (Monkey > pony)"
