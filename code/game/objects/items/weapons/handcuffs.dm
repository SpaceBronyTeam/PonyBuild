...
/obj/item/weapon/handcuffs/proc/place_handcuffs(var/mob/living/carbon/target, var/mob/user)
	...
	if (ispony(target))
		var/mob/living/carbon/pony/H = target
...
var/last_chew = 0
/mob/living/carbon/pony/RestrainedClickOn(var/atom/A)
	if (A != src) return ..()
	...
	if (H.zone_sel.selecting.name != "mouth") return
	if (H.wear_mask) return
	if (istype(H.wear_suit, /obj/item/clothing/suit/straight_jacket)) return

	var/datum/organ/external/O = H.organs_by_name[H.hand?"l_forehoof":"r_forehoof"]
	if (!O) return
