...
//	causeerrorheresoifixthis
	...
	ex:
	sprite_sheets = list(
		"Pegasus" = 'icons/cat/are/bad'
		)
	If index term exists and icon_override is not set, this sprite sheet will be used.
	*/
	var/list/sprite_sheets = null
	var/icon_override = null  //Used to override hardcoded clothing dmis in pony clothing proc.
...
/obj/item/attack_hand(mob/user as mob)
	if (!user) return
	if (hasorgans(user))
		var/datum/organ/external/temp = user:organs_by_name["r_forehoof"]
		if (user.hand)
			temp = user:organs_by_name["l_forehoof"]
...
/obj/item/verb/verb_pickup()
	...
	if(!usr.canmove || usr.stat || usr.restrained() || !Adjacent(usr))
		return
	if((!istype(usr, /mob/living/carbon)) || (istype(usr, /mob/living/carbon/brain)))//Is ponyoid, and is not a brain
...
/obj/item/add_blood(mob/living/carbon/pony/M as mob)
	if (!..())
		return 0
...
modules/mob/living/carbon/pony/life.dm if you die, you will be zoomed out.
*/
...
/obj/item/proc/zoom(var/tileoffset = 14,var/viewsize = 9) //tileoffset is client view offset in the direction the user is facing. viewsize is how far out this thing zooms. 7 is normal view
	...
	if(usr.stat || !(istype(usr,/mob/living/carbon/pony)))
