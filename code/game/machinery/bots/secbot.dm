...
/obj/machinery/bot/secbot/Topic(href, href_list)
	...
	if(lasercolor && (istype(usr,/mob/living/carbon/pony)))
	...
	switch(mode)
		...
		//			O << "<span class='game say'><span class='name'>[src]</span> beeps, \"Backup requested! Suspect has evaded arrest.\""
			...
			if(target)		// make sure target exists
				// We re-assess pony targets, before bashing their head in, in case their credentials change
				if(istype(target, /mob/living/carbon/pony))
...
/obj/machinery/bot/secbot/on_assess_perp(mob/living/carbon/pony/perp)
	if(lasercolor)
...
/obj/machinery/bot/secbot/proc/laser_check(mob/living/carbon/pony/perp, var/lasercolor)
	var/target_suit
	var/target_weapon
	var/threat = 0
