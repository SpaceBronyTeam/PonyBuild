...
/obj/machinery/iv_drip/var/mob/living/carbon/pony/attached = null
...
/obj/machinery/iv_drip/MouseDrop(over_object, src_location, over_location)
	...
	if(in_range(src, usr) && ispony(over_object) && get_dist(over_object, src) <= 1)
...
/obj/machinery/iv_drip/process()
	...
	if(src.attached)
		...
		if(!(get_dist(src, src.attached) <= 1 && isturf(src.attached.loc)))
			...
			src.attached:apply_damage(3, BRUTE, pick("r_foreleg", "r_foreleg"))
			src.attached = null
			src.update_icon()
			return
	...
	if(src.attached && src.beaker)
		...
		// Take blood
			...
			var/mob/living/carbon/pony/T = attached
			...
			// If the pony is losing too much blood, beep.
