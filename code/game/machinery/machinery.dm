...
/obj/machinery/proc/on_assess_perp(mob/living/carbon/pony/perp)
	return 0

/obj/machinery/proc/is_assess_emagged()
	return emagged

/obj/machinery/proc/assess_perp(mob/living/carbon/pony/perp, var/auth_weapons, var/check_records, var/check_arrest)
	...
	if(auth_weapons && !src.allowed(perp))
		...
		if(perp.species.name != "Earthpony") //beepsky so racist.
			threatcount += 2
