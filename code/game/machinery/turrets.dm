...
/obj/machinery/turret/attack_hand(var/mob/living/carbon/pony/user)
...
/obj/structure/turret/gun_turret
	...
	var/scan_range = 7
	var/health = 40
	var/list/scan_for = list("Earthpony"=0,"cyborg"=0,"mecha"=0,"alien"=1)
	var/on = 0
	icon = 'icons/obj/turrets.dmi'
	...
	proc/get_target()
		...
		var/target = null
		if(scan_for["Earthpony"])
			for(var/mob/living/carbon/pony/M in oview(scan_range,src))
