//Restores our verbs. It will only restore verbs allowed during lesser (monkey) form if we are not pony
/mob/proc/make_changeling(var/mode="alone", var/list/roles)
	...
	//var/lesser_form = !ispony(src)


	//var/temp_dna = dna
	//var/temp_species = species.name

	var/changeling_caste = "Changeling Queen"
	for(var/datum/mind/changeling in roles)
		if(changeling.current.species.name == "Changeling Queen")
			changeling_caste = pick(200;"Changeling Infiltrator", 30;"Changeling Drone", 10;"Changeling Sentinel", 1;"Changeling Princess")
	change_mob_type( /mob/living/carbon/pony , null, null, 1, changeling_caste)
	...
	mind.changeling.base_dna = dna
	mind.changeling.base_species = species.name


	if(changeling_caste == "Changeling Queen")
		ticker.mode.allowed_species += temp_species

	if(!(temp_species in ticker.mode.allowed_species))
		temp_species = pick(ticker.mode.allowed_species-"Alicorn")

	mind.changeling.absorbed_species |= temp_species
	mind.changeling.absorbed_dna[temp_dna] = new/datum/changeling_shape(temp_dna, temp_species)
	...
	if(mode!="alone")	goto infiltrator_mode
...
/mob/proc/changeling_change_species()
	...
	if(!istype(H))
		src << "<span class='warning'>We may only use this power while in pony form.</span>"
		return
...
/mob/proc/changeling_absorb_dna()
	...
	var/mob/living/carbon/pony/T = G.affecting
	if(!istype(T))
	...
	for(var/stage = 1, stage<=3, stage++)
		switch(stage)
			...
			if(3)
				...
				var/datum/organ/external/affecting = T.get_organ(src.zone_sel.selecting.name)
...
/mob/proc/changeling_lesser_transform()
	...
	var/mob/living/carbon/pony/O = new /mob/living/carbon/pony( src )
...
/mob/proc/changeling_unstun()
	...
	var/mob/living/carbon/pony/C = src
	C.stat = 0
	C.SetParalysis(0)
	C.SetStunned(0)
	C.SetWeakened(0)
...
//Prevents AIs tracking you but makes you easily detectable to the pony-eye.
...
/mob/proc/changeling_transformation_sting()
	...
	if(!T)	return 0
	if((HUSK in T.mutations) || (!ispony(T) && !ismonkey(T)))
...
/mob/proc/changeling_extract_dna_sting()
	...
	var/mob/living/carbon/pony/T = changeling_sting(40, /mob/proc/changeling_extract_dna_sting)
	if(!T)	return 0
