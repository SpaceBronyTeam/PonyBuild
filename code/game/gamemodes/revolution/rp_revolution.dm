...
/datum/game_mode/revolution/rp_revolution/post_setup()
	...
	for(var/datum/mind/rev_mind in head_revolutionaries)
		...
		rev_mind.current.verbs += /mob/living/carbon/pony/proc/RevConvert
...
/datum/game_mode/revolution/rp_revolution/greet_revolutionary(var/datum/mind/rev_mind, var/you_are=1)
	...
	for(var/i = 0, i < 2, i++)
		...
		var/mob/living/carbon/pony/M = get_nt_opposed()
...
/datum/game_mode/revolution/rp_revolution/add_revolutionary(datum/mind/rev_mind)
	...
	var/mob/living/carbon/pony/H = rev_mind.current//Check to see if the potential rev is implanted
	if(!is_convertible(H))
		return 0
...
/mob/living/carbon/pony/proc/RevConvert()
	set name = "Rev-Convert"
	set category = "IC"
	var/list/Possible = list()
	for (var/mob/living/carbon/pony/P in oview(src))
		if(!stat && P.client && P.mind && !P.mind.special_role)
			Possible += P
	if(!Possible.len)
		src << "\red There doesn't appear to be anyone available for you to convert here."
		return
	var/mob/living/carbon/pony/M = input("Select a person to convert", "Viva la revolution!", null) as mob in Possible
...
/datum/game_mode/revolution/rp_revolution/process()
	...
	if(tried_to_add_revheads < world.time)
		...
		if(active_revs == 0)
			...
			var/added_heads = 0
			for(var/mob/living/carbon/pony/H in world) if(H.client && H.mind && H.client.inactivity <= 10*60*20 && H.mind in revolutionaries)
				...
				H.verbs += /mob/living/carbon/pony/proc/RevConvert
