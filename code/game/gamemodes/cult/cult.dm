...
/proc/is_convertable_to_cult(datum/mind/mind)
	...
	if(istype(mind.current, /mob/living/carbon/pony) && (mind.assigned_role in list("Captain", "Chaplain")))	return 0
...
/datum/game_mode/cult/post_setup()
	...
	if("sacrifice" in objectives)
		...
		if(!possible_targets.len)
			for(var/mob/living/carbon/pony/player in player_list)
...
/datum/game_mode/proc/equip_cultist(mob/living/carbon/pony/mob)
	if(!istype(mob))
		return
...
/datum/game_mode/cult/grant_runeword(mob/living/carbon/pony/cult_mob, var/word)
	if (!word)
		if(startwords.len > 0)
...
/datum/game_mode/proc/grant_runeword(mob/living/carbon/pony/cult_mob, var/word)
	if(!cultwords["travel"])
		runerandom()
	if (!word)
		word=pick(allwords)
...
/datum/game_mode/cult/proc/get_unconvertables()
	var/list/ucs = list()
	for(var/mob/living/carbon/pony/player in mob_list)
