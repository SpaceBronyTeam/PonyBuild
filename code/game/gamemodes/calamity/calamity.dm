...
/datum/game_mode/calamity/proc/get_role_candidates(var/role)
	...
	switch(role)
		...
		if("slug")
			possible_antags = get_players_for_role(BE_CHANGELING_INFILTRATOR)
		if("cult")
...
/datum/game_mode/calamity/proc/spawn_ninja(var/list/candidates)
	...
	for(var/datum/mind/player in candidates)
		...
		if(player.current && !(istype(player.current,/mob/living/carbon/pony))) return 0
...
/datum/game_mode/calamity/proc/spawn_borers(var/list/candidates)
	...
	spawn(10)
		for(var/datum/mind/player in candidates)
			...
			borers |= player
			var/mob/living/carbon/pony/target_host = pick(possible_hosts)
