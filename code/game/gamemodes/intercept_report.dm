...
/datum/intercept_text/proc/pick_fingerprints()
	var/mob/living/carbon/pony/dude = src.pick_mob()
...
/datum/intercept_text/proc/get_suspect()
	var/list/dudes = list()
	for(var/mob/living/carbon/pony/man in player_list) if(man.client && man.client.prefs.nanotrasen_relation == "Opposed")
		dudes += man
...
/datum/intercept_text/proc/build_traitor(datum/mind/correct_person)
	...
	var/mob/living/carbon/pony/H = get_suspect()
	if(!H) return
