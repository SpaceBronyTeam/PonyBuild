...
/*
Malf AIs/silicons aren't added. Monkeys aren't added. Messes with objective completion. Only ponys are added.
*/
	...
	spawn(300)
		...
		var/mob/living/carbon/pony/new_ninja = create_space_ninja(pick(ninjastart.len ? ninjastart : latejoin))
		...
		for(var/mob/living/carbon/pony/xeno in player_list)
			if(istype(xeno.species,/datum/species/changeling))
				xeno_list += xeno
		...
		if(assign_mission)
			...
			if(xeno_list.len>3)//If there are more than three ponyoid xenos on the station, time to get dangerous.
				//Here we want the ninja to murder all the queens. The other aliens don't really matter.
				var/xeno_queen_list[] = list()
				for(var/mob/living/carbon/pony/xeno_queen in xeno_list)
				...
				if(xeno_queen_list.len&&side=="face")//If there are queen about and the probability is 50.
					for(var/mob/living/carbon/pony/xeno_queen in xeno_queen_list)
			...
			if(sent_strike_team&&side=="heel"&&antagonist_list.len)//If a strike team was sent, murder them all like a champ.
				...
				if(commando_list.len)//If there are living commandos still in play.
					for(var/mob/living/carbon/pony/commando in commando_list)
						var/datum/objective/assassinate/ninja_objective = new
						ninja_objective.owner = ninja_mind
...
/proc/generate_ninja_directive(side)
	...
	switch(rand(1,19))
		...
		if(5)
			directive += "We are currently negotiating with NanoTrasen Central Command. Prioritize saving pony lives over ending them."
		if(6)
		...
		if(14)
			var/xenorace = pick("Unicorn","Pegasus", "Alicorn")
...
/proc/create_space_ninja(obj/spawn_point)
	var/mob/living/carbon/pony/new_ninja = new(spawn_point.loc)
...
/mob/living/carbon/pony/proc/create_mind_space_ninja()
	mind_initialize()
...
/mob/living/carbon/pony/proc/equip_space_ninja(safety=0)//Safety in case you need to unequip stuff for existing characters.
