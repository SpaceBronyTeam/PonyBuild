...
datum/objective
	...
	proc/find_target()
		...
		for(var/datum/mind/possible_target in ticker.minds)
			if(possible_target != owner && ispony(possible_target.current) && (possible_target.current.stat != 2))
	...
	proc/find_target_by_role(role, role_type=0)//Option sets either to check assigned role or special role. Default to assigned.
		for(var/datum/mind/possible_target in ticker.minds)
			if((possible_target != owner) && ispony(possible_target.current) && ((role_type ? possible_target.special_role : possible_target.assigned_role) == role) )
...
datum/objective/mutiny
	...
	check_completion()
		if(target && target.current)
			if(target.current.stat == DEAD || !ispony(target.current) || !target.current.ckey)
				return 1
...
datum/objective/mutiny/rp
	...
	check_completion()
		...
		if(target && target.current)
			...
			if(target.current.stat == DEAD || target.current:handcuffed || !ispony(target.current))
				return 1
...
datum/objective/anti_revolution/execute
	...
	check_completion()
		if(target && target.current)
			if(target.current.stat == DEAD || !ispony(target.current))
				return 1
			return 0
		return 1
...
datum/objective/anti_revolution/demote
	...
	check_completion()
		if(target && target.current && istype(target,/mob/living/carbon/pony))
...
datum/objective/harm
	...
	check_completion()
		...
		if(target && target.current && istype(target.current, /mob/living/carbon/pony))
			if(target.current.stat == DEAD)
				return 0

			var/mob/living/carbon/pony/H = target.current
...
datum/objective/download
	...
	check_completion()
		...
		if(istype(owner.current,/mob/living/carbon/pony))
			var/mob/living/carbon/pony/H = owner.current
			S = H.back
...
datum/objective/capture
	...
	check_completion()//Basically runs through all the mobs in the area to determine how much they are worth.
		...
		for(var/mob/living/carbon/pony/M in A) // ponys (and subtypes).
	...
	proc/gen_amount_goal(var/lowbound = 4, var/highbound = 6)
		...
		if (ticker)
			...
			if (ticker.current_state == GAME_STATE_SETTING_UP)
				...
				for(var/mob/living/carbon/pony/P in player_list)
...
datum/objective/heist/kidnap
	choose_target()
		...
		for(var/datum/mind/possible_target in ticker.minds)
			if(possible_target != owner && ispony(possible_target.current) && (possible_target.current.stat != 2) && (possible_target.assigned_role != "MODE"))
				possible_targets += possible_target
	...
	check_completion()
		if(target && target.current)
			...
			for(var/mob/living/carbon/pony/M in A)
				if(target.current == M)
