...
/datum/game_mode/proc/declare_completion()
	...
	if(surviving_ponys > 0)
		feedback_set("survived_pony",surviving_ponys)
	if(surviving_total > 0)
		feedback_set("survived_total",surviving_total)
	if(escaped_ponys > 0)
		feedback_set("escaped_pony",escaped_ponys)
	if(escaped_total > 0)
...
/datum/game_mode/proc/send_intercept()
	...
	for(var/mob/living/carbon/pony/man in player_list) if(man.client && man.mind)
		// NT relation option
...
proc/get_nt_opposed()
	var/list/dudes = list()
	for(var/mob/living/carbon/pony/man in player_list)
		if(man.client)
