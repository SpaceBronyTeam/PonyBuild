/*	Note from Carnie:
	...
	-	When creating a new mob for an existing IC character (e.g. cloning a dead guy or borging a brain of a pony)
...
datum/mind
	...
	proc/edit_memory()
		...
		var/text = ""
		var/mob/living/carbon/pony/H = current
		if (istype(current, /mob/living/carbon/pony) || istype(current, /mob/living/carbon/monkey))
			/** Impanted**/
			if(istype(current, /mob/living/carbon/pony))
...
//				text += "<br>All the changelings are dead! Restart in [round((changeling.TIME_TO_GET_REVIVED-(world.time-changeling.changelingdeathtime))/10)] seconds."
		...
		if (istype(current, /mob/living/carbon))
			...
			text = "<i><b>[text]</b></i>: "
			if (istype(current, /mob/living/carbon/pony))
				text += "<a href='?src=\ref[src];monkey=healthy'>healthy</a>|<a href='?src=\ref[src];monkey=infected'>infected</a>|<b>pony</b>|other"
				...
				if(found)
					text += "<a href='?src=\ref[src];monkey=healthy'>healthy</a>|<b>INFECTED</b>|<a href='?src=\ref[src];monkey=pony'>pony</a>|other"
				else
					text += "<b>HEALTHY</b>|<a href='?src=\ref[src];monkey=infected'>infected</a>|<a href='?src=\ref[src];monkey=pony'>pony</a>|other"

			else
				text += "healthy|infected|pony|<b>OTHER</b>"
		...
		if (((src in ticker.mode.head_revolutionaries) || \
			...
			istype(current,/mob/living/carbon/pony)      )
			...
			switch (new_obj_type)
				if ("assassinate","protect","debrain", "harm", "brig")
					...
					for(var/datum/mind/possible_target in ticker.minds)
						if ((possible_target != src) && istype(possible_target.current, /mob/living/carbon/pony))
			...
			switch(href_list["revolution"])
				if("clear")
					...
					if(src in ticker.mode.head_revolutionaries)
						...
						ticker.mode.update_rev_icons_removed(src)
						special_role = null
						current.verbs -= /mob/living/carbon/pony/proc/RevConvert
			...
			switch(href_list["monkey"])
				...
				if("Earthpony")
					...
					if (istype(M))
						...
						log_admin("[key_name(usr)] attempting to ponyize [key_name(current)]")
						message_admins("\blue [key_name_admin(usr)] attempting to ponyize [key_name_admin(current)]")
						var/obj/item/weapon/dnainjector/m2h/m2h = new
						var/obj/item/weapon/implant/mobfinder = new(M) //hack because ponyizing deletes mind --rastaf0
						src = null
						m2h.inject(M)
...
//pony
/mob/living/carbon/pony/mind_initialize()
	..()
