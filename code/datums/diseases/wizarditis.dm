/datum/disease/wizarditis
	...
	affected_species = list("Earthpony")
	curable = 1
	permeability_mod = 0.75
	desc = "Some speculate, that this virus is the cause of Wizard Federation existance. Subjects affected show the signs of mental retardation, yelling obscure sentences or total gibberish. On late stages subjects sometime express the feelings of inner power, and, cite, 'the ability to control the forces of cosmos themselves!' A gulp of strong, manly spirits usually reverts them to normal, ponylike, condition."
	severity = "Major"
...
/datum/disease/wizarditis/proc/spawn_wizard_clothes(var/chance = 0)
	if(istype(affected_mob, /mob/living/carbon/pony))
		var/mob/living/carbon/pony/H = affected_mob
		if(prob(chance))
		...
		if(prob(chance))
			if(!H.type_in_hands(/obj/item/weapon/staff, H.hand))
				H.drop_active_hand()
				H.put_in_active_hand( new /obj/item/weapon/staff(H) )
			return
	return
